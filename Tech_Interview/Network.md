...
### REST API의 개념
1.REST API의 개념
Representational State Transfer. "웹에 존재하는 모든 자원(이미지, 동영상, DB 자원)에 고유한 URI를 부여해 활용하는 것"으로, 자원을 정의하고 자원에 대한 주소를 지정하는 방법론을 의미한다고 한다. RESTful API는 REST의 특징을 지키면서 API를 제공하는 것을 의미한다.
2000년도에 로이 필딩 (Roy Fielding)의 박사학위 논문에서 최초로 소개. 로이 필딩은 HTTP의 주요 저자 중 한 사람으로서 웹의 장점을 최대한 활용할 수 있는 아키텍처로써 REST를 발표.
