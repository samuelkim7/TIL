...
### 트랜잭션
- 정의: 데이터베이스 처리의 논리적 단위를 형성하는 실행 프로그램. 한번에 모두 수행되어야 할 일련의 연산들의 집합
- Commit: 하나의 트랜잭션에 대한 작업이 성공적으로 끝났고 데이터베이스가 다시 일관된 상태에 있을 때, 모든 갱신사항을 데이터베이스에 안전하게 저장하며 이후 취소되지 않는다.
- Rollback: 하나의 트랜잭션 처리가 비정상적으로 종료되어 데이터베이스의 일관성을 깨뜨렸을 때, 해당 작업의 일부가 정상적으로 처리되었더라도 트랜잭션의 원자성을 구현하기 위해 이 트랜잭션이 행한 모든 연산을 취소하는 것
  - 시스템 로그: 시스템은 데이터베이스 항목들의 값에 영향을 미치는 모든 트랜잭션 연산들을 추적하기 위해서 로그를 유지한다. 이 기록 정보를 사용하여 트랜잭션 실패시 롤백을 수행한다.

#### ACID (트랜잭션의 성질)
- Atomicity(원자성): 한 트랜잭션은 하나의 원자적 수행 단위임. 즉 트랜잭션의 연산은 데이터베이스에 모두 반영되든지 아니면 전혀 반영되지 않아야 한다. 즉 어느 하나라도 오류가 발생하면 트랜잭션 전부가 취소되어야 함
- Consistency(일관성): 트랜잭션의 실행이 성공적으로 완료되면 언제나 데이터베이스를 하나의 일관된 상태에서 또 다른 일관된 상태로 변환한다.
- Isolation(격리성): 둘 이상의 트랜잭션이 동시에 병행 실행되는 경우 어느 하나의 트랜잭션 실행 중에 다른 트랜잭션의 연산이 끼어들 수 없다. 즉 하나의 트랜잭션이 X에 대해 연산을 수행하고 있다면 다른 트랜잭션은 X에 대해 참조하거나 연산을 수행할 수 없고 앞의 트랜잭션이 끝날 때까지 대기해야 한다.
  - 만약 이러한 동시성 제어가 되지 않을 경우 하나의 트랜잭션에서 X를 갱신한 후 값을 저장하기 전에, 다른 트랜잭션에서 X를 읽고 연산을 수행할 경우 최초 갱신사항이 손실되는 문제가 발생한다. 또한 하나의 트랜잭션에서 X를 갱신한 후 차후 연산에서 오류가 생기는 경우에서 다른 트랜잭션이 X의 임시값을 읽을 경우에도 문제가 된다. 그리고 하나의 트랜잭션에서 X를 갱신하고 저장하기 전에, 다른 트랜잭션에서 X와 관련된 집합 연산을 수행하는 경우에 집합 연산의 값이 틀리게 된다. 
- Durability(영속성): 성공적으로 완료된 트랜잭션에 의해 데이터베이스 가해진 변경은 그 데이터베이스에 영속해야 한다. 그 변경은 어떠한 고장에도 손실되지 않아야 한다.  
<a href="#top">TOP</a>
...
### 인덱스를 사용하는 이유와 장단점
- RDBMS에서 해당 TABLE의 컬럼을 색인화(별도의 파일로 저장)하여 검색시 해당 TABLE의 레코드를 full scan하는 것이 아니라 인덱스 파일을 검색하여 검색속도를 향상시킴
- 정렬되어 있어서 빠른 탐색이 가능함. 새로운 데이터가 추가, 삭제, 변경되면 이에 상응하는 인덱스를 추가, 삭제, 변경해야 한다. 
- SELECT 쿼리의 WHERE 절이나 JOIN 예약어를 사용했을 때에만 인덱스를 사용하게 됨
- 단점: 인덱스를 만들기 위한 추가적인 디스크 공간이 필요하고, 인덱스 생성 자체에도 시간이 소요된다. 빈번한 데이터 추가, 삭제, 변경 시 이와 함께 인덱스 수정이 일어나야 해서 데이터베이스 성능을 저하시킨다. 참고로 인덱스의 경우 수정의 개념이 없고 delete와 insert를 통해 수정이 수행됨

#### 인덱스 사용 지침
- WHERE 절에서 사용되는 컬럼이나 JOIN에 자주 사용되는 컬럼을 인덱스로 만든다.
- 외래키가 사용되는 컬럼에는 되도록 인덱스를 생성해 준다.
- 데이터의 중복도가 높은 열은 인덱스로 만들어도 효용이 없다.
- 갱신이 빈번하게 이루어지는 attribute/relation은 인덱스를 만드는 것을 피해야 함
- 대량의 데이터를 삽입하는 경우에는 모든 인덱스를 제거하고 데이터 삽입이 끝난 후에 인덱스를 다시 생성하는 것이 좋다.

#### 인덱스의 구조
- TABLE의 경우 컬럼이 여러 개이며 데이터는 정렬되지 않고 입력된 순서대로 들어간다.
- 인덱스의 경우 컬럼이 key 컬럼(인덱스를 생성하라고 지정한 컬럼)과 ROWID 컬럼 두개 이루어져 있고 정렬되어 있음
...
### ORM
- ORM(Object Relational Model)은 OOP의 객체와 DB의 테이블을 매핑하는 기술. 사물을 추상화시켜 이해하려는 OOP적 사고방식과 DataModel을 정형화하여 관리하려는 RDB 사이를 연결할 계층의 역할로 제시된 패러다임으로 RDB의 모델을 OOP에 Entity 형태로 투영시키는 방식을 사용한다. 각 테이블 또는 구분하고자하는 데이터 단위로 객체를 구현하고, 데이터간의 관계를 형성한다. SQL query가 아닌 직관적인 코드를 통해 데이터 조작을 가능하게 해준다.
- 장점: 객체 지향적인 코드로 인해 더 직관적이며 비즈니스 로직에 더 집중할 수 있게 도와주어서 생산성이 크게 향상됨. DBMS에 대한 종속성이 줄어들어서 DBMS 교체에도 비교적 적은 리스크와 시간이 소요됨
- 단점: 복잡한 query는 ORM으로 표현하는 데 한계가 있으며 raw query에 비해 성능이 떨어짐. ORM 설계를 잘못 구현한 경우 속도가 저하되거나 데이터의 일관성이 무너질 수 있음. DBMS 고유 기능을 이용하기 어려우며 프로시져가 많은 시스템에서는 ORM의 객체 지향적인 장점을 활용하기 어려움
