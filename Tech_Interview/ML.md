...
- 공분산: 확률변수 X와 Y에 대해 X가 변할 때 이에 따라 Y가 변하는 정도를 나타내는 값. 두 확률변수의 관계를 보여주는 값. 편차의 곱의 기댓값으로 구하며 두 사건이 독립일 경우 공분산은 0임
...
- 상관계수: 측정단위와 상관 없이 두 확률 변수의 상관성을 보여주는 값. 공분산을 두 확률변수가 갖는 표준편차의 으로 나눈 값
...
- 중심극한정리  
모집단이 평균이 m이고 표준편차가 s인 임의의 분포를 이룬다고 할 때, 이 모집단으로부터 추출된 표분의 크기 n이 충분히 크다면 표본 "평균들의 분포"는 평균이 m이고 표준편차가 s / sqrt(n) 인 정규분포에 근접한다. 수집한 표본들의 통계량을 이용해서 모집단의 모수를 추정할 수 있는 확률적 근거를 마련해 줌
...
![image](https://user-images.githubusercontent.com/65876994/155079818-adaa46fc-028a-4e9c-a67d-c39755751a3d.png)
- P-Value
...
- 차원 축소  
  ...
  - PCA: 주성분 추출을 통해 차원 축소를 수행하는 기법. 데이터를 축에 사영했을 때 가장 높은 분산을 가지는 데이터의 축을 찾아 그 축(주성분)으로 차원을 축소하는 것. 높은 분산을 갖는 축(주성분)을 찾는 이유는 정보의 손실을 최소화 하기 위함. 다른 의미에서 분산이 크다는 것은 그만큼 주성분이 원래 데이터의 분포를 잘 설명해줄 수 있음을 의미함
    ...
    - 공분산행렬의 고유벡터가 바로 주성분이 됨. 그 고유벡터 중에서 고유값이 큰 것 부터 
- 불균형 데이터를 위한 샘플링 기법
  - undersampling
...
- 주요 인자 탐지
  - 여러 개의 센서 관측치를 기반으로 제품의 정상 / 불량을 판단할 때 어떤 센서가 주요 인자인지를 탐지하는 문제
  - 각각의 센서 관측치 데이터에 대해 정상 / 불량 클래스를 나누어서 가설검정을 하는 것이 필요함
  - 가설검정
    - 관측된 데이터 (샘플)를 기반으로 가설을 검정함. 샘플 데이터에서 얻은 결론은 맞을 수도 있고 틀릴 수도 있음
    - 귀무가설 (Null Hypothesis, H0): 기존의 사실. Equal
    - 대립가설 (Alternative Hypothesis, H1): 새롭게 주장하고 싶은 사실. Not Equal
    - 검정통계량: 귀무가설이 참이라는 가정 아래 얻은 통계량 (function of samples)
    - 다중가설검정: 모든 가설을 동시에 검정하면 문제가 생기므로 Family Wise로 생각하여 검정. 이것도 인자의 개수가 10개 이상으로 많아지면 문제가 됨
    - False Discovery Rate (FDR): 기각된 가설 중 잘못 기각된 가설이 차지하는 평균 비율. 우선 각각의 인자에 대한 P-Value를 구하고 이들을 작은 값부터 sorting한다. 그런 다음 각 인자에 대해 (rank / N) * alpha 값을 구한다. 그리고 해당 값이 P-Value 보다 큰 경우 중 rank의 최대값 까지를 경계로 하여 위는 Significant, 아래는 Not Significant로 나눈다.
    - 반복 샘플링 기반 P-Value: 데이터가 정규분포를 따르지 않는 경우 분포에 대한 가정 없이 P-Value를 구하는 방법. 두 클래스를 나누어서 통계량을 구한 뒤에 shuffle을 수행하고 통계량을 구하는 것을 N번 반복한다. 그런 다음 기존의 통계량과 shiffle 후의 통계량을 비교해서 기존의 통계량 보다 큰 개수 (x)가 적을 수록 통계적으로 유의함. 이 때의 P-Value = x / N. 이를 통해서 반복 샘플링 기반 FDR을 계산할 수도 있음
